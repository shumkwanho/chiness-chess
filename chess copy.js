...
const initialState = [
    ...
    [new Ma('balck'), 0, 4, 0, 0, 0, 0, 14, 0, new Ma('red')],
    [6, 0, 0, 7, null, 0, 17, 0, 0, 16],
...
function chessMovement(e) {
    const newX = Math.floor(e.offsetX / 80)
    const newY = Math.floor(e.offsetY / 80)
    const chessId = chessState[x][y]
    const pickedChess1 =  chessState[x][y]
    // const pickedChess = new Ma('black')
    const pickedChess = new Ma('black')
    ...
    //pick empty
    if (chessId == null) return
    // not your turn
    if (pickedChess.color != turn) return
    // pick up
    if (pickFlag == false) {
        pickFlag = true
    ...
    //same position
    if (x == chessPicked.x && y == chessPicked.y) {
        chessPicked = []
        pickFlag = false
        drawCanvas()
        return
    ...
    const isMoveLegal = pickedChess.logic(newX,newY)
    ...
    chessState[chessPicked[0]][chessPicked[1]] = null
    chessState[x][y] = pickedChess
    pickedChess.x = newX
    pickedChess.y = newY

    const isWin = chessId == chessName.jiang || chessId == chessName.shuai
    const isWin2 = pickedChess instanceof Ma || pickedChess instanceof Ma
...
class Chess {
    ...
    draw(){
        ...
        ctx.shadowColor = "transparent"
        drawText(padding + 10 + unitWidth * x, padding - 10 + unitHeight * y, obj[chessState[x][y]].text, obj[chessState[x][y]].color)
        drawText(padding + 10 + unitWidth * x, padding - 10 + unitHeight * y, text, color)
    }
}
...
class Ma extends Chess {
    ...
    /**check the move is good
     * @p 
     * 
     * @return bool -> is the move good
     */
    logic(x, y, color){
        jiangLogic(x, y, this.color)
    }
}
